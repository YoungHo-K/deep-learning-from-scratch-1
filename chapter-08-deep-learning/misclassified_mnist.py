...
for index in range(int(x_test.shape[0] / batch_size)):
    tx = x_test[index * batch_size: (index + 1) * batch_size]
    tt = t_test[index * batch_size: (index + 1) * batch_size]
...
mis_pairs = {}
for index, val in enumerate(classified_ids == t_test):
    if not val:
        ax = fig.add_subplot(4, 5, current_view, xticks=[], yticks=[])
        ax.imshow(x_test[index].reshape(28, 28), cmap=plt.cm.gray_r, interpolation='nearest')
        mis_pairs[current_view] = (t_test[index], classified_ids[index])
